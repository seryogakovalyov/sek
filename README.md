...
Каждый раз, когда агент что-то делает в проекте — отправляет запрос, получает ответ, использует инструмент, находит баг, принимает решение — SEK это запоминает. Не в виде сырых логов, а в виде компактных наблюдений (observations).

В следующей сессии, когда агент сталкивается с похожей задачей, SEK сам находит и подкладывает релевантный предыдущий опыт. Агенту не нужно вспоминать — он просто получает готовое знание.

**Важно:** SEK ничего не делает вместо агента. Он не пишет код, не запускает тесты, не принимает решений. Он только хранит и выдаёт опыт, когда это уместно.
...
Каждая новая сессия агента видит всё, что накоплено предыдущими сессиями. Единственное условие — MCP сервер должен быть настроен на один и тот же `--project`.
